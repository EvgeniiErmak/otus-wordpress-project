#!/bin/bash
# setup-slave.sh — ФИНАЛЬНАЯ ВЕРСИЯ С ГАРАНТИРОВАННЫМ ПОЯВЛЕНИЕМ ЛОГОВ В KIBANA
...
# ======================== FILEBEAT + ГАРАНТИРОВАННАЯ НАСТРОЙКА KIBANA ========================
log "Filebeat + принудительная настройка Kibana..."
...
# Принудительно генерируем много логов
log "Генерируем тестовые логи для быстрого появления индекса..."
for i in {1..30}; do
    echo "[$(date)] Test log from Nginx on slave #$i" >> /var/log/nginx/access.log
    echo "[$(date)] Test log from Apache on slave #$i" >> /var/log/apache2/access.log
    echo "[$(date)] Test log from MySQL on slave #$i" >> /var/log/mysql/error.log
done

sleep 25

# Автоматическая настройка Kibana
log "Автоматическая настройка Kibana (Index Pattern)..."
...
echo "   → 30 тестовых логов сгенерировано"
echo "   → Перейдите в Discover → выберите 'logs-*' — логи должны появиться"
echo "=================================================================="
