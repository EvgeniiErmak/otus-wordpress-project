  ...
  "panels": [
    {
      "type": "stat",
      "title": "CPU Usage %",
      "targets": [{ "expr": "100 - avg(irate(node_cpu_seconds_total{mode=\"idle\",job=\"node_exporter\"}[5m])) * 100" }]
    },
    {
      "type": "stat",
      "title": "Used RAM (GB)",
      "targets": [{ "expr": "(node_memory_MemTotal_bytes{job=\"node_exporter\"} - node_memory_MemAvailable_bytes{job=\"node_exporter\"}) / 1024 / 1024 / 1024" }]
    },
    {
      "type": "stat",
      "title": "System Load 5m",
      "targets": [{ "expr": "node_load5{job=\"node_exporter\"}" }]
    },
    {
      "type": "stat",
      "title": "Uptime (days)",
      "targets": [{ "expr": "(node_time_seconds{job=\"node_exporter\"} - node_boot_time_seconds{job=\"node_exporter\"}) / 86400" }]
    }
  ],
...
log "Grafana + дашборд OTUS Node Exporter Simple настроены"
