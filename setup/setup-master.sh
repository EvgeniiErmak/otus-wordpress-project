#!/bin/bash
# setup/setup-master.sh — ПОЛНЫЙ ФАЙЛ С ИСПРАВЛЕННЫМ ДАШБОРДОМ
...
# Ждём запуска Grafana и Prometheus
sleep 20
  ...
  "dashboard": {
    ...
    "panels": [
      ...
      },
      {
        ...
        "type": "stat",
        "targets": [{"expr": "node_memory_MemAvailable_bytes / 1073741824"}],
      ...
      },
      {
        ...
        "type": "stat",
        "targets": [{"expr": "100 * (1 - node_filesystem_avail_bytes{mountpoint=\"/\"} / node_filesystem_size_bytes{mountpoint=\"/\"})"}],
...
curl -X POST http://admin:admin@localhost:3000/api/dashboards/db \
  ...
  -d @/tmp/otus-dashboard.json || true
