#!/bin/bash
# setup/setup-master.sh — ПОЛНЫЙ ФАЙЛ БЕЗ СОКРАЩЕНИЙ. ВСЁ ИСПРАВЛЕНО.
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Установка и настройка Nginx reverse proxy..."
...
wp config create --dbname=wordpress --dbuser=wpuser --dbpass=WpPassword2026Strong! --dbhost=localhost --locale=ru_RU --force --skip-check --allow-root
wp core install --url=http://192.168.88.168 --title="Мой личный блог" --admin_user=admin --admin_password=AdminPassword2026Strong! --admin_email=admin@example.com --locale=ru_RU --skip-email --allow-root
...
# Создаём дашборд
sleep 15
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Создаём дашборд в Grafana..."
cat > /tmp/otus-dashboard.json << 'EOD'
{
  "dashboard": {
    "uid": "otus-wordpress",
    "title": "OTUS WordPress Infrastructure",
    "tags": ["wordpress", "otus"],
    "timezone": "browser",
    "panels": [
      {"title": "CPU Usage %", "type": "stat", "targets": [{"expr": "100 - (avg(irate(node_cpu_seconds_total{mode=\"idle\"}[5m])) * 100)"}], "gridPos": {"h": 4, "w": 6, "x": 0, "y": 0}},
      {"title": "Memory Available (GB)", "type": "stat", "targets": [{"expr": "node_memory_MemAvailable_bytes / 1073741824"}], "gridPos": {"h": 4, "w": 6, "x": 6, "y": 0}},
      {"title": "Load Average (1m)", "type": "stat", "targets": [{"expr": "node_load1"}], "gridPos": {"h": 4, "w": 6, "x": 12, "y": 0}},
      {"title": "Disk Usage Root %", "type": "stat", "targets": [{"expr": "100 * (1 - node_filesystem_avail_bytes{mountpoint=\"/\"} / node_filesystem_size_bytes{mountpoint=\"/\"})"}], "gridPos": {"h": 4, "w": 6, "x": 18, "y": 0}}
    ]
  },
  "overwrite": true
}
...
curl -X POST http://admin:admin@localhost:3000/api/dashboards/db -H "Content-Type: application/json" -d @/tmp/otus-dashboard.json || true
rm -f /tmp/otus-dashboard.json
...
echo "               Дашборд 'OTUS WordPress Infrastructure' создан"
