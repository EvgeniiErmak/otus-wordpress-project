#!/bin/bash
# =====================================================
# setup-master.sh — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ
# Автоматическая установка всего стека OTUS WordPress Project
# =====================================================
...
log "Перезапуск Prometheus с очисткой старых данных..."
...
# Скачиваем дашборд и фиксим job
wget -q -O /var/lib/grafana/dashboards/node-exporter-full.json \
    https://grafana.com/api/dashboards/1860/revisions/37/download || true

# Простая фиксация job (заменяем в JSON)
sed -i 's/"job": *"[^"]*"/"job": "node_exporter"/g' /var/lib/grafana/dashboards/node-exporter-full.json || true
...
echo "=================================================================="
echo "Проверь в Grafana дашборд Node Exporter Full — метрики должны появиться."
echo "Если всё ещё N/A — обнови страницу (F5) несколько раз."
echo "=================================================================="
